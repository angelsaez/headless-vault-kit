...
against the live vault, and the same index numbers again. That is the only test of an off-site
hook: an upload's exit code says a command succeeded, and a restore says a backup exists.
